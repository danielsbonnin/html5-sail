...
class MiniMapComponent extends CanvasComponent {
    draw(ctx, args={width: 100, height: 100, boatPos: {x: 0, y: 0}, mapWidth: 100, mapHeight: 100, boatSize: 10}) {
        ...
        let adjustedWidth = args.width - (2*args.boatSize) - (2*lineWidth);
        let adjustedHeight = args.height - (2*args.boatSize) - (2*lineWidth);
        ...
        ctx.save();
        ctx.translate(x + lineWidth + args.boatSize, y+lineWidth + args.boatSize);
        ctx.rotate(-boat.angle);
        ctx.scale(0.35, 0.35);
        HullComponent.draw(ctx, args.boatSize);
        SailComponent.draw(ctx, args.boatSize);
        ctx.restore();
        ctx.restore();
    }
}